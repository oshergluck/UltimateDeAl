  ...
  const formatFullPhoneNumber = () => {
    ...
    const fetchPaymentSymbol = async () => {
      ...
      const AddReviewButton = async () => {
        ...
        alert("You're not the owner of the invoice");
        setIsLoading(false);
      }
                    ...
                    <div className="mt-[20px] rounded-2xl border border-white/10 bg-black/20 p-4">
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        <input
                          ...
                          type="text"
                          placeholder="Enter Invoice Number (e.g. 5)"
                          value={receipt}
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                          {/* Review Button */}
                          <button
                            className="flex-1 rounded-2xl px-6 py-3 font-extrabold shadow-lg bg-white text-blue-700 hover:shadow-xl transition"
                          ...
                          <TransactionButton
                            className="flex-1 !rounded-2xl !px-6 !py-3 !font-extrabold !shadow-lg !transition !bg-red-500/20 !text-red-400 !border !border-red-500/30 hover:!bg-red-500/30"
                            transaction={async () => {
                              if(!receipt) {
                              ...
                              // הכנת הקריאה לפונקציית ה-refundClient בחוזה
                              return prepareContractCall({
                                contract: storeContract1, // החוזה של החנות
                                method: "function refundClient(uint256 _receiptId)",
                                params: [receipt],
                              });
                            }}
                            onTransactionConfirmed={(txReceipt) => {
                              alert("Refund successful! Your tokens have been returned and the invoice NFT burned.");
                              refreshPage(); // רענון כדי לעדכן מלאי/סטטוס
                            }}
                            onError={(error) => {
                              console.error("Refund error:", error);
                              if (error.message.includes("Refund period expired")) {
                                alert("Sorry, the 14-day refund period has expired for this invoice.");
                              } else if (error.message.includes("Not authorized")) {
                                alert("You are not the owner of this invoice.");
                              } else {
                        ...
                        <p className="text-[10px] text-white/40 italic">
                          * Refunds are available for 14 days from purchase. Requires burning the digital invoice.
                        </p>
                      </div>
                    </div>
