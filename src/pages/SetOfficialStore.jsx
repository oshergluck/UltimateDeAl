...
const SetOfficialStore = () => {
  const navigate = useNavigate();
  const [isLoading, setIsLoading] = useState(false);
  const { addOfficialStoreDetails } = useStateContext();
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await addOfficialStoreDetails({ ...form });
      setIsLoading(false);
    }
    catch (error) {
      alert(error);
  ...
  return (
    <div className="linear-gradient rounded-[15px] flex justify-center items-center flex-col sm:p-10 p-2 mt-[50px]">
      ...
      <div className="flex justify-center items-center p-[16px] sm:min-w-[380px] rounded-[2px]">
        <h1 className="font-epilogue font-bold sm:text-[25px] text-[18px] leading-[38px] text-white drop-shadow-md">Set Official Store</h1>
      </div>
...
export default SetOfficialStore;