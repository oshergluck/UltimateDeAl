...
export default function DeployVotes() {
  const account = useActiveAccount();
  const [formData, setFormData] = useState({
    token: "",
    minAmountOfTokens: "",
    proposalDurationInDays: ""
  });
  ...
  const handleDeploy = async () => {
    ...
    if (!formData.minAmountOfTokens.trim()) {
      setError("Please enter Minimum Amount Of Tokens to vote");
      return;
    }
    if (!formData.proposalDurationInDays.trim()) {
      setError("Please enter duration of proposal in days");
      return;
    }
    ...
    try {
      const params = {
        ...
        minAmountOfTokens: formData.minAmountOfTokens,
        proposalDurationInDays : formData.proposalDurationInDays
      };
    ...
    <div className="min-h-screen linear-gradient1 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Deploy ESH Voting</h1>
          <p className="text-white">Create and deploy your custom voting contract on Base</p>
        </div>
          ...
          <div className="p-8">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-gray-500">ETH Address</p>
              </div>

              {/* Token Symbol */}
              <div>
                <label className="block text-sm font-semibold text-white mb-2">
                Minimum Tokens For Voting <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  name="minAmountOfTokens"
                  value={formData.minAmountOfTokens}
                  onChange={handleInputChange}
                  placeholder="Minimum amount of tokens"
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none text-gray-900"
                  maxLength="10"
                />
                <p className="mt-1 text-xs text-gray-500">Minimum amount of tokens</p>
              </div>
              {/* Token Symbol */}
              <div>
                <label className="block text-sm font-semibold text-white mb-2">
                Duration Of Proposal <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  name="proposalDurationInDays"
                  value={formData.proposalDurationInDays}
                  onChange={handleInputChange}
                  placeholder="Days"
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none text-gray-900"
                  maxLength="10"
                />
                <p className="mt-1 text-xs text-gray-500">Duration In Days</p>
              </div>
